...
const DriverHome: React.FC = () => {
          ...
          <div className="space-y-4 w-full">
            {rideRequests.map((request) => (
              <div
                ...
                className={`flex items-center p-4 shadow-md rounded-md cursor-pointer ${
                    selectedRide?.id === request.id ? 'bg-gray-200' : 'bg-background-light'
                  }`}
              >
  ...
  <span className="text-gray-500">Pickup Time:</span> {
    format(new Date(request.pickupTime), 'eeee, MMMM do yyyy, h:mm a')
  }
</p>                    <a
